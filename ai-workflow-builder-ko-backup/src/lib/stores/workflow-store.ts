 ...
 * 노드, 엣지, 실행 상태를 포함합니다.
 * 
 * 리팩토링됨: 템플릿은 /lib/templates/에서 관리
 */
...
import { workflowTemplates, TemplateType } from '@/lib/templates/workflow-templates';
import { createWorkflowId, createNodeId } from '@/lib/utils';

// ============================================
// 자동 저장 설정
// ============================================
const AUTO_SAVE_KEY = 'jogwajang-workflow';
...
export const useWorkflowStore = create<WorkflowState>()(
  persist(
    (set, get) => ({
      ...
      setWorkflowName: (name) => set({ workflowName: name, lastSaved: new Date().toISOString() }),
      ...
      loadWorkflow: (json) => {
        try {
          ...
          set({
            workflowId: data.id || createWorkflowId(),
      ...
      loadTemplate: (templateType) => {
        const template = workflowTemplates[templateType];
        if (template) {
          set({
            workflowId: createWorkflowId(),
