    ...
    <div className="space-y-4">
      ...
      {data.preset === 'dart' && (
        <>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">회사명, 종목코드, 또는 기업코드</label>
            <input
              type="text"
              ...
              placeholder="예: 삼성전자, 005930, 아이티아이즈"
              className="w-full p-2 border border-gray-300 rounded-lg text-sm"
            />
            <p className="mt-1 text-xs text-gray-500">
              ✅ <strong>회사명</strong> (삼성전자, 삼전, 카카오, 토스...)<br />
              ✅ <strong>종목코드</strong> (005930, 372800...)<br />
              ✅ <strong>비상장사</strong>도 지원 (쿠팡, 당근마켓...)<br />
              입력 노드에서 받기: <code className="bg-gray-100 px-1 rounded">{'{{input}}'}</code>
            </p>
          </div>
          <div>
          ...
          {data.presetConfig?.reportType === 'financial' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">조회 기간</label>
              <select
                value={data.presetConfig?.yearCount || 1}
                onChange={(e) => updateNodeData(nodeId, {
                  presetConfig: { ...data.presetConfig, yearCount: Number(e.target.value) }
                })}
                className="w-full p-2 border border-gray-300 rounded-lg text-sm"
              >
